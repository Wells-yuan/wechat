...
}, "只能包括中文字、英文字母、数字和下划线和空格");  
  
// 数字精度验证
jQuery.validator.addMethod("accuracy", function (value, element, params) { 
	if(isNaN(params)) {
		console.error("参数错误，验证的精度(accuracy)只能为数字");
		return false;
	}
	if(isNaN(value)) {
		return false;
	...
	if(typeof(params) == undefined || params == 0) {
		...
		var regxStr = "^\\d+(\\.\\d{1,"+params+"})?$";
		var regX = new RegExp(regxStr);
	}
	return this.optional(element) || (regX.test(value));
}, jQuery.validator.format("最多保留小数点后{0}位的数值"));  

// 身份证号码验证       
jQuery.validator.addMethod("checkEnergyCard", function(value, element) {
    return this.optional(element) || checkEnergyCard(value);       
}, "请正确填写您的身份证号码");
...
},"请正确填写时间格式如：2013-01-01");

jQuery.validator.addMethod("isDownNow",function(value,element){
	var flag = false;
...
function checkEnergyCard(allowancePersonValue){
 ...
 {
	 //jAlert("您输入的身份证号码长度不正确，请重新输入！",'提示');
	 return false;
 }
 //检查省份
 else if(checkProvince(allowancePersonValue) === false)
 {
	 //jAlert("您输入的身份证号码省份不正确，请重新输入！",'提示');
	 return false;
 }
 //校验生日
 ...
 {
	 //jAlert("您输入的身份证号码校验位不正确，请重新输入！",'提示');
  	 return false;
 }else{
  return true;
 }
